...
public class HLServerInfo {
  ...
  public static ArrayList<PlayerInfo> getPlayers(int localPort, String ipStr, int port, int infoType, int queryType,
          int gameType) {
    /**
     * On some operating systems the following code will return the wrong number of bytes in Unicode to UTF-8 conversion
     * 
    ...
    byte[] buf = null;
    try {
      buf = Util.getInfo(localPort, ipStr, port, "UA2S_PLAYER", infoType, queryType, gameType).getBytes("ISO-8859-1");
      if (buf.length == 0) {
        return null;
      }
